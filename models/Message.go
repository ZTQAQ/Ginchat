...
func Chat(writer http.ResponseWriter, request *http.Request) {
	...
	Id := query.Get("userId")
	userId, _ := strconv.ParseInt(Id, 10, 64) //把string转换成int64类型
	...
	node := &Node{
		...
		DataQueue:     make(chan []byte, 50),      //字节流，之后可以直接发送，不需要序列化
		GroupSets:     set.New(set.ThreadSafe),
	}
	//3. 用户关系
	//4. 要把每个客户端（userid）和每个连接绑定，不加锁可能出现多个客户端绑定到同一个连接
	rwLocker.Lock()
...
func recvProc(node *Node) {
	for {
		...
		if msg.Type == 3 {
			currentTime := uint64(time.Now().Unix())
			node.Heartbeat(currentTime) //更新心跳时间
		} else { //发送消息
			dispatch(data)
...
func sendMsg(userId int64, msg []byte) {
	...
	// 将消息存储到 Redis 中，以便后续用户登录时能够获取到离线消息
	var key string
	//生成一个相同的键
	...
	//score用来排序消息，cap获取切片的容量
	score := float64(cap(res)) + 1
	//向指定的有序集合中添加一个成员，并设置该成员的"分数"
